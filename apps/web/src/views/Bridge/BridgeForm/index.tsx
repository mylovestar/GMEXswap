...
const getPID = (symbol?: string, chainId?: number) => {
  ...
  if (symbol === "KNB") {
    return chainId === ChainId.BSC ? {pid: 1, isNative: true} : {pid: 1, isNative: false}
  }
  if (symbol === "XKR") {
...
export const BridgeForm: React.FC<Record<string, never>> = () => {
  ...
  const sourceChain = chainId === ChainId.ETHEREUM ? chainId : ChainId.BSC
  const targetChain = sourceChain === ChainId.ETHEREUM ? ChainId.BSC : ChainId.ETHEREUM
